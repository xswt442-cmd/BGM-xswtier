...
<Popover>
	<PopoverTrigger class="min-h-11 rounded-sm transition-opacity hover:opacity-80 sm:min-h-0">
		<StatusChip
			label="KEY"
			value={apiToken.hasToken ? 'TOKEN SET' : 'NO TOKEN'}
			iconClass="icon-[pixelarticons--lock]"
		/>
	</PopoverTrigger>
	{#snippet content()}
		<div class="grid gap-3">
			...
			<div class="flex items-center justify-between gap-2">
				<a href="https://next.bgm.tv/demo/access-token" target="_blank" class="text-xs text-primary hover:underline">
					{m.get_token()}
				</a>
