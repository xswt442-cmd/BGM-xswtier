<script lang="ts">
	...
	import StatusChip from './control/StatusChip.svelte';
	...
	function buildSearchParams(offset = 0): SearchParams {
		// 来源 + 类型合并进 meta_tags（地区「日本」仅快捷入口加，常规搜索不加）
		const metaTags = [...(source ? [source] : []), ...(type ? [type] : [])];
	...
	function addAllResults() {
		// 检索池全部加入排名池（按 id 去重，已加的不重复）
		const existing = selectedIds;
		const fresh = results.filter((i) => !existing.has(i.id));
		if (fresh.length > 0) selected = [...selected, ...fresh];
	}
	...
	function goToTier() {
		// 基于排名池，非 results
		if (selected.length === 0 || !mode) return;
		itemLoader.clear();
		itemLoader.seedLoaded(selected);
...
<div class="grid gap-3">
	...
	<div class="flex flex-wrap items-center gap-2">
		...
		<StatusChip label={m.filter_region_label()} value={m.filter_region_japan()} />
	</div>
	...
	<div class="grid grid-cols-1 gap-2 sm:grid-cols-3">
		<div class="grid gap-1">
			...
			<select
				class="font-pixel h-8 w-full border-2 border-border bg-card px-1 text-[10px] text-foreground"
				bind:value={platform}
			>
