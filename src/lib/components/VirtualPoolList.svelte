...
<div
	class="border-t border-border/50 px-2 py-0.5 text-right font-pixel text-[8px] text-muted-foreground"
...
{:else if !virtual}
	<div
		...
		class="grid min-w-0 grid-cols-1 gap-1.5 overflow-y-auto p-1.5 sm:grid-cols-2"
	...
	>
		{#each items as item (item.id)}
			<PoolRow
				...
				actionVariant={actionVariant(item)}
...
{:else}
	<div
		bind:this={scrollElement}
		class="min-w-0 overflow-y-auto p-1.5"
		style:height={`min(${maxHeight}, ${Math.max(VIRTUAL_POOL_ROW_HEIGHT, $virtualizer.getTotalSize())}px)`}
		data-testid={`${testid}-list`}
	>
		<div class="relative w-full" style:height={`${$virtualizer.getTotalSize()}px`}>
			{#each $virtualizer.getVirtualItems() as virtualRow (virtualRow.key)}
				<div
					class="absolute left-0 top-0 grid w-full grid-cols-1 gap-1.5 sm:grid-cols-2"
