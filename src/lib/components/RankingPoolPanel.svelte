...
<section class="min-w-0 border-2 border-border bg-card/60" data-testid="ranking-pool-panel">
	<div class="flex items-center justify-between gap-2 border-b-2 border-border px-2 py-1">
		...
		<div class="flex items-center gap-2">
			...
			<Button variant="ghost" size="sm" class="font-pixel h-5 px-2 text-[9px]" onclick={() => { searchPool.clear(); selection.clear(); }} disabled={items.length === 0}>
				{m.pool_delete_all()}
			</Button>
		</div>
	</div>
	{#if items.length > 0}
		<div class="flex flex-wrap items-center gap-1.5 border-b-2 border-border px-2 py-1">
			...
			<Button variant="ghost" size="sm" class="font-pixel h-7 px-2 text-[8px]" onclick={() => selectAllMutable(selection, items)}>{m.pool_select_all()}</Button>
			<Button variant="ghost" size="sm" class="font-pixel h-7 px-2 text-[8px]" onclick={() => selection.clear()} disabled={selection.size === 0}>{m.pool_clear_selection()}</Button>
	...
	{/if}
	<VirtualPoolList items={items} {active} testid="pool-row" checked={(id) => selection.has(id)} onToggle={toggle} actionVariant={() => 'destructive'} actionLabel={() => m.pool_delete()} onAction={(item) => remove(item.id)} maxHeight="55svh" />
</section>
