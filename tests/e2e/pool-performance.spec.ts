...
test('500 search and ranking items stay virtualized while batch operations remain atomic', async ({ page }) => {
	await page.route('https://api.bgm.tv/**', async (route) => {
		...
		await route.fulfill({
			...
			json: { data, total: 500, limit: 20, offset },
		});
	});
	...
	await page.getByTestId('search-row-list').evaluate((element) => {
	...
	await page.route('https://api.bgm.tv/**', async (route) => {
		...
		if (url.pathname.endsWith('/v0/indices/200/subjects')) {
			...
			return route.fulfill({
				status: 200,
				contentType: 'application/json',
				json: { data, total: 200, limit: 50, offset },
			});
		}
		const match = url.pathname.match(/\/v0\/subjects\/(\d+)$/);
		if (match)
			return route.fulfill({ status: 200, contentType: 'application/json', json: mockSubject(Number(match[1])) });
		return route.abort();
	});
	await openStart(page);
